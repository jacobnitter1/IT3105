...
class SplitGD():
    ...
    def gen_loss(self,features,targets,avg=False):
        predictions = self.model(tf.convert_to_tensor(features,dtype=tf.float32))  # Feed-forward pass to produce outputs/predictions
        loss = self.model.loss_functions[0](tf.convert_to_tensor(targets,dtype=tf.float32),predictions)
